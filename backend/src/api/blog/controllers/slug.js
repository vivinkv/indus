            ...
            if (existingBlog) {
              ...
              const processContent = async (content) => {
                ...
                while ((match = imgRegex.exec(content)) !== null) {
                  ...
                  const response = await axios.get(
                    originalUrl.startsWith('http') ? originalUrl : `https://indususedcars.com/${originalUrl}`,
                  ...
                  if (uploadResponse.data[0]) {
                    const newUrl = `${process.env.STRAPI_URL || 'http://localhost:1337'}${uploadResponse.data[0].url}`;
